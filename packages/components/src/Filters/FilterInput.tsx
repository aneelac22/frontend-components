...
const FilterInput: React.FC<FilterInputProps> = ({ addRemoveFilters, param = '', filters = {}, type = 'checkbox', value, id, label }) => {
  ...
  const handleChange = () => {
    ...
    addRemoveFilters && addRemoveFilters(value, param, type, checked);
  };
  if (type !== 'radio') {
    return <Checkbox aria-label={param} id={id} isChecked={checked} label={label} onChange={handleChange} value={value} ouiaId={id} />;
  }
  return (
    <Radio
